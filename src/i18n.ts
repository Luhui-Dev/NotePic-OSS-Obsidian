...
const en = {
  ...
  panel: {
    ...
    emptyNoImages: "No images found in this note.",
    localPrefix: "Local · vault",
    targetArrow: "→",
    ossBadge: "OSS",
    uploadRow: "Upload this image",
    uploadingRow: "Uploading…",
    filter_all: "All",
    ...
    downloadDone: (downloaded: number, failed: number) =>
      `NotePic OSS: downloaded ${downloaded} image(s)` + (failed > 0 ? `, failed ${failed}` : ""),
    downloadFolderError: (message: string) => `NotePic OSS: unable to choose download folder — ${message}`,
    ...
    assets: {
      ...
      noRefs: "no refs",
      openReference: "Open referencing file",
      showMoreReferences: (n: number) => `+ ${n} more`,
      hideReferences: "Show less",
  ...
  downloadFailure: {
    title: (n: number) => `Download failed for ${n} image(s)`,
    noReason: "(no reason given)",
    copy: "Copy failures",
    copied: "Copied ✓",
    close: "Close",
...
const zh: typeof en = {
  ...
  panel: {
    ...
    upload: "上传",
    downloadN: (n: number) => `下载 ${n} 项`,
    download: "下载",
    downloading: (done: number, total: number) => `正在下载 ${done} / ${total}`,
    downloadDone: (downloaded: number, failed: number) =>
      `NotePic OSS：已下载 ${downloaded} 张图片` + (failed > 0 ? `，失败 ${failed} 张` : ""),
    downloadFolderError: (message: string) => `NotePic OSS：无法选择下载文件夹 — ${message}`,
    ...
    assets: {
      ...
      noRefs: "无引用",
      openReference: "打开引用文件",
      showMoreReferences: (n: number) => `再显示 ${n} 项`,
      hideReferences: "收起",
      badgeReferenced: "已引用",
